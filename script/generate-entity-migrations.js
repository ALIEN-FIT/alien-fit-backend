...
function resolveReferenceTable(refModel) {
    if (!refModel)
        return null;
    if (typeof refModel === 'string') {
        return refModel;
    }
    const asRecord = refModel;
    ...
    if (typeof asRecord.name === 'string') {
        const maybeModel = sequelize.models[asRecord.name];
        if (maybeModel) {
...
function topologicalSort(dependencies) {
    ...
    for (const [node, deps] of dependencies.entries()) {
        for (const dep of deps) {
            if (!dependencies.has(dep) || dep === node)
                continue;
...
function writeMigrations() {
    ...
    for (const [tableName, model] of tableToModel.entries()) {
        ...
        for (const attribute of Object.values(model.getAttributes())) {
            const refTable = resolveReferenceTable(attribute?.references?.model);
            if (refTable)
                deps.add(refTable);
        }
    ...
    for (const [index, tableName] of orderedTables.entries()) {
        ...
        const enumQueries = Object.entries(attributes)
            .filter(([, attribute]) => attribute?.type?.key === 'ENUM')
            .map(([attributeName, attribute]) => queryGenerator.pgEnum(tableName, attributeName, attribute.type))
            .filter(Boolean);
